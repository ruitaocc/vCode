...
static BitStream *QRinput_getBitStream(QRinput *input,int *paddingoffset)
{
	BitStream *bstream;
	int ret;
	...
	//printf("bstream length:%d\n",(bstream->length+7)/8);
 	*paddingoffset = (bstream->length+7)/8;
	if(input->mqr) {
...
unsigned char *QRinput_getByteStream(QRinput *input,int* paddingoffset)
{
	BitStream *bstream;
	unsigned char *array;

	bstream = QRinput_getBitStream(input,paddingoffset);
	if(bstream == NULL) {
		return NULL;
	}
