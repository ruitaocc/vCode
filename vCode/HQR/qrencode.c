...
static int image[74][2]={
	...
	{14+2+8,17+8-5},
	{15+2+8,17+8-5},
	{15+2+8,21+8-5},{15+3+8-2,21+8-5},{15+3+8-2,21+8-1-5},
	{16+2+8,18+8-5},
	{16+2+8,19+8-5},
	{16+2+8,20+8-5}
};
int isInImage(int i,int j){
	int k,res=0;
	for(k=0;k<74;k++){
		if(image[k][0]==i&&image[k][1]==j){res=1;break;}
	}
	return res;
}
...
static QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw->datacode = QRinput_getByteStream(input,&paddingOffset);//!!!
...
/**
 * Return a code pointer(byte).
 * This function can be called iteratively.
 * @param raw raw code.
 * @return code
 */
static unsigned char* QRraw_getCodePtr(QRRawCode *raw)
{
	int col, row;
	unsigned char *ret;
...
static QRcode *QRcode_encodeMask(QRinput *input, int mask,int *maskImage)
{
	int width, version;
	int spec[5], ret;
	int isMask0 = 0;
	...
	raw = QRraw_new(input);
	if(raw == NULL) return NULL;
	raw_sec = QRraw_new(input);
	if(raw_sec == NULL) return NULL;

	version = raw->version;
	width = QRspec_getWidth(version);
	paddingArea =(int *)malloc(sizeof(int)*width*width);
	memset(paddingArea,0,sizeof(int)*width*width);
	...
	for(i=0; i<raw->dataLength; i++) {
		...
		for(j=0; j<8; j++) {
			...
			r_r = (raw->count - 1 ) % raw->blocks;
			r_c = (raw->count - 1 ) / raw->blocks;
			if(r_r<raw->b1){
				m_countor = r_r * raw->rsblock[0].dataLength + r_c ;
			}
			else {
				m_countor = r_r * raw->rsblock[0].dataLength + r_c + (r_r-raw->b1);
			}

			if(m_countor>=paddingOffset &&(maskImage!=NULL)){
				//this point is padding data area;
				paddingArea[f_row*width+f_cul] = 1;

				isImage = maskImage[f_row*width+f_cul];
				isMask0=(f_row+f_cul)%2;
				isTestbit = (bit & (*codePtr))!=0;
				
				if(isImage){
					isMask0?(*p = 0x03):(*p = 0x02);
					if(isMask0){*codePtr = ((*codePtr)| bit);};//*->1
					if(!isMask0){*codePtr = ((*codePtr)&(~bit));};//*->0
				}else{
					isMask0?(*p = 0x02):(*p = 0x03);
					if(!isMask0){*codePtr = ((*codePtr)| bit);};//*->1
					if(isMask0){*codePtr = ((*codePtr)&(~bit));};//*->0
				}
			}else{
				*p = 0x02 | ((bit & (*codePtr)) != 0);//!!! 0x02 represent this is data or ecc module
			}
			bit = bit >> 1;
		}
...
QRcode *QRcode_encodeInput(QRinput *input,int * maskImage)
{
	if(input->mqr) {
		return QRcode_encodeMaskMQR(input, -1);
	} else {
		return QRcode_encodeMask(input, -1,maskImage);
	}
}

static QRcode *QRcode_encodeStringReal(const char *string, int version, QRecLevel level, int mqr, QRencodeMode hint, int casesensitive,int * maskImage)
{
	QRinput *input;
	QRcode *code;
	int ret;
	...
	code = QRcode_encodeInput(input,maskImage);
	QRinput_free(input);

	return code;
}

QRcode *QRcode_encodeString(const char *string, int version, QRecLevel level, QRencodeMode hint, int casesensitive,int * maskImage)
{
	return QRcode_encodeStringReal(string, version, level, 0, hint, casesensitive,maskImage);
}

QRcode *QRcode_encodeStringMQR(const char *string, int version, QRecLevel level, QRencodeMode hint, int casesensitive)
{
	return QRcode_encodeStringReal(string, version, level, 1, hint, casesensitive,NULL);
}

static QRcode *QRcode_encodeDataReal(const unsigned char *data, int length, int version, QRecLevel level, int mqr,int * maskImage)
{
	QRinput *input;
	QRcode *code;
	int ret;
...
{
	return QRcode_encodeDataReal(data, size, version, level, 0,NULL);
}
...
QRcode *QRcode_encodeString8bit(const char *string, int version, QRecLevel level)
{
	...
	return QRcode_encodeDataReal((unsigned char *)string, strlen(string), version, level, 0,NULL);
}

QRcode *QRcode_encodeDataMQR(int size, const unsigned char *data, int version, QRecLevel level)
{
	return QRcode_encodeDataReal(data, size, version, level, 1,NULL);
}
...
QRcode_List *QRcode_encodeInputStructured(QRinput_Struct *s)
{
	...
	while(list != NULL) {
		...
		tail->code = QRcode_encodeInput(list->input,NULL);
		if(tail->code == NULL) {
			goto ABORT;
		}
		list = list->next;
